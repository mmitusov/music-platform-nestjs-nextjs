...
P.S. Причем стоит заметить, то app.module.ts является точкой входа (ключевым модулем). Поэтому если мы создаем другие/новые модули под какие-то задачи, мы должны убедиться что они указаны и ипрортированы в наш ключевой модуль, от которого уже будет разростатся логика приложения. После чего в других модулях мы можем указывать новые контроллеры, которые будут отрабатывать по новому иникальному пути. Такм отбразом мы можем создавать кучу разных API: /users, /auth, /test и т.д.
...
Также мы можем использовать сервис родительского модуля в любом другом внешнем модуле (его контроллерах и сервисах). Но как это сделать? Есть несколько способов:
1. Воспользоваться декоратором Global() для того сервиса который мы хотим сделать глобальным
...
***JWT аутентификация***
Для аутентификации вместо голой JWT логики, советуют пользоваться Pasport JS.
...
```
***Валидация данных с запроса***
Для валидации данных с запроса мы  можем использовать либо Built-in pipes, либо Class validator. Class validator - это доп библиотека которую нужно устанавливать.
Class validator нужно использовать с глобальной Middleware - ValidationPipe, чтобы валидатор работал для всех частей кода.
```
app.useGlobalPipes (new ValidationPipe({ 
  whitelist: true
})):
```
`whitelist: true` предотвращает чтобы сервер принимал что либо лишнее, что не указано в DTO. И уберает всю лишнею инфу из запроса.
