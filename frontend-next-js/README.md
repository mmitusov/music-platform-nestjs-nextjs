...
Почистим сперва сгенерированый макет. Будем начинать с нуля. Удалим все из папки src и создадим там 2 пустые папки pages и styles. styles - все наши стили. pages - будет хранить роутинг на все наши страницы. Также в pages создадим и _app.jsx. Но не нужно путать _app.jsx и index.jsx.
...
Далее начнем создавать остальные страницы приложения:
...
в компоненте tracks/, при нажатии на любую точку карточки трека, нас будет переводить на соответствующий трек. Но внутри карточки у нас есть такие елементы как кнопка плей/пауза. И как сделать так, чтобы только при нажатии на этот вложенный елемент, нас не переводило на новую страницу, а все остальные вложенные елементы продолжали отрабатывать как следует? Для этого воспользуемся методом синтетического ивента, как - preventDefault() или stopPropagation():
```
...
Сперва разберемся как работать с аудио файлами в браузере. Мы можем это делать при помощи объкта - `audioObj = new Audio()`. Однако не забываем, что так как мы работаем с NextJS, то мы находимся в среде сервера, а не в браузере. А потому если мы из коробки пропишем new Audio() - то этот объект работать не будет. Для этого воспользуемся хуком `useEffect()`, который как только наш сайт впервые загрузиться в браузере - будет создаваться/отрабатывать new Audio().
...
```
Также добавим условие на то, что если у нас нет текущего активного/выбраного трека, то нижний плеер (Player.tsx) тогда не будет отображаться на экране:
```
...
Далее реализуем клиент серверную составляющею и будем получать наши треки с сервера. Так как в Player.tsx у нас храниться состояние только одного трека, то мы дополнительно создадим отдельный слайс для хранения перечня всех треков что мы получаем с бекенда - tracksSlice.ts.
...
P.S. Создав асинхронный екшен, лучше перезагрузить/пересобрать сайт, иначе Webpack может начать тупить и выдавать ошибки вроде: TypeError: Cannot read properties of undefined (reading 'getState').
...
Сбор и хранение данных внутри каджого отдельного шага Steppera мы будем реализовывать пр помощи onChangeFieldHandler. Однако это также можно было бы также сделать и при момощи созданого кастомного хука - useInput. Хоть мы нигде и не применяем useInput, я решил его оставить как это интерестный пример того, как можно реализовать работу с инпутами.

После того как мы собрали все данные, отправляем их при помощи - const formData = new FormData(). И после этого перебрасываем юзера на страницу с треками. P.S. Так как мы сперва дожидаемся респонса о том, что трек уже создан. То когда мы переходим на '/tracks', то там уже сразу подгружается обновленные данные с бека, включая загруженный трек.
...
Также добавим возможность удалять трек и все связанные с ним данные. Для этого создадим еще один асинхронный екшен - deleteOneTrack.

Иногда, когда мы проходимся циклом по массиву, Реакт на TypeScript-те может ругаться, что он не уверен что ".map()" применяется именно к массиву. При этом выдавая ошибку вроде - TypeError: map() is not a function in React. Решением является то, что мы просто предварительно проверяем, работаем мы с массивом или нет - Array.isArray(tracks). Например:
```
